...
def trying_to_add_audio(original_movie_path, no_snd_movie_path, output_path, tmp_dir):
    if os.path.isfile(original_movie_path):
        ...
        if os.path.isfile(sound_path):
            ...
            subprocess.call(
                "ffmpeg -i " + no_snd_movie_path + " -i " + sound_path + " -c:v copy -c:a aac -map 0:v:0 -map 1:a:0 " + output_path,
                shell=True)
            return True
...
def ebsynth_utility_stage7(dbg, project_args, blend_rate, export_type, is_invert_mask):
    ...
    black_img = np.zeros_like(
        cv2.imread(os.path.join(out_dirs[cur_clip]['path'], str(start).zfill(number_of_digits) + ".png")))
    ...
    for i in range(start, end + 1):
        ...
        if i in range(out_dirs[next_clip]['startframe'], out_dirs[next_clip]['endframe'] + 1):
            ...
            back_rate = (i - out_dirs[next_clip]['startframe']) / max(1, (
                        out_dirs[cur_clip]['endframe'] - out_dirs[next_clip]['startframe']))
