'''
Some image processing functions, for example:
- resize image
- create mask from image (using CLIPSeg)
- apply mask to image
'''
...
def create_mask_clipseg(input_dir, output_dir, clipseg_mask_prompt, clipseg_exclude_prompt, clipseg_mask_threshold,
                        mask_blur_size, mask_blur_size2):
...
def create_mask_transparent_background(input_dir, output_dir, tb_use_fast_mode, tb_use_jit, st1_mask_threshold):
    ...
    if os.path.isfile(bin_path) or os.path.isfile(bin_path + ".exe"):
        subprocess.call(
            bin_path + " --source " + input_dir + " --dest " + output_dir + " --type map" + fast_str + jit_str,
            shell=True)
    else:
        subprocess.call(
            "transparent-background --source " + input_dir + " --dest " + output_dir + " --type map" + fast_str + jit_str,
            shell=True)
...
def ebsynth_utility_stage1(project_args, frame_width, frame_height, st1_masking_method_index, st1_mask_threshold,
                           tb_use_fast_mode, tb_use_jit, clipseg_mask_prompt, clipseg_exclude_prompt,
                           clipseg_mask_threshold, clipseg_mask_blur_size, clipseg_mask_blur_size2, is_invert_mask):
    print("stage1")
    print("")

    if st1_masking_method_index == 1 and (not clipseg_mask_prompt):
        print("Error: clipseg_mask_prompt is Empty")
        return
    ...
    if is_invert_mask:
        if os.path.isdir(frame_path) and os.path.isdir(frame_mask_path):
            print("Skip as it appears that the frame and normal masks have already been generated.")
            return
    ...
    if os.path.isdir(frame_path):
        ...
        subprocess.call(
            "ffmpeg -ss 00:00:00  -y -i " + original_movie_path + " -qscale 0 -f image2 -c:v png " + png_path,
            shell=True)
    ...
    if frame_mask_path:
        if st1_masking_method_index == 0:
            create_mask_transparent_background(frame_path, frame_mask_path, tb_use_fast_mode, tb_use_jit,
                                               st1_mask_threshold)
            ...
            if not os.path.isdir(tb_tmp_path):
                ...
                create_mask_transparent_background(frame_path, tb_tmp_path, tb_use_fast_mode, tb_use_jit,
                                                   st1_mask_threshold)
            create_mask_clipseg(frame_path, frame_mask_path, clipseg_mask_prompt, clipseg_exclude_prompt,
                                clipseg_mask_threshold, clipseg_mask_blur_size, clipseg_mask_blur_size2)
...
def ebsynth_utility_stage1_invert(frame_mask_path, inv_mask_path):
    print("stage 1 create_invert_mask")
    print("")

    if not os.path.isdir(frame_mask_path):
        print(frame_mask_path + " not found")
        print("Normal masks must be generated previously.")
        return
