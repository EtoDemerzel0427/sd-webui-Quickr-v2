...
import json
import re
from deforum_helpers.calculator import CalcParser, ParseError
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        with gr.Column(variant='panel'):
            with gr.Column():
                ...
                generation_test = gr.Checkbox(False,
                                              label="Generation TEST!!(Ignore Project directory and use the image and mask specified in the main UI)")

            with gr.Accordion("Mask option"):
                mask_mode = gr.Dropdown(choices=["Normal", "Invert", "None", "Don't Override"], value="Normal",
                                        label="Mask Mode(Override img2img Mask mode)")
                inpaint_area = gr.Dropdown(choices=["Whole picture", "Only masked", "Don't Override"], type="index",
                                           value="Only masked", label="Inpaint Area(Override img2img Inpaint area)")
            ...
            with gr.Accordion("ControlNet option"):
                controlnet_weight = gr.Slider(minimum=0.0, maximum=2.0, step=0.01, value=0.5,
                                              label="Control Net Weight")
                controlnet_weight_for_face = gr.Slider(minimum=0.0, maximum=2.0, step=0.01, value=0.5,
                                                       label="Control Net Weight For Face")
            ...
            with gr.Accordion("Loopback option"):
                img2img_repeat_count = gr.Slider(minimum=1, maximum=30, step=1, value=1,
                                                 label="Img2Img Repeat Count (Loop Back)")
            ...
            with gr.Accordion("Auto Tagging option"):
                auto_tag_mode = gr.Dropdown(choices=["None", "DeepDanbooru", "CLIP"], value="None",
                                            label="Auto Tagging")
                add_tag_to_head = gr.Checkbox(False, label="Add additional prompts to the head")
                add_tag_replace_underscore = gr.Checkbox(False,
                                                         label="Replace '_' with ' '(Does not affect the function to add tokens using add_token.txt.)")
                gr.HTML(value="<p style='margin-bottom: 0.7em'>\
            ...
            with gr.Accordion("Face Crop option"):
                ...
                with gr.Row():
                    face_detection_method = gr.Dropdown(choices=["YuNet", "Yolov5_anime"], value="YuNet",
                ...
                face_crop_resolution = gr.Slider(minimum=128, maximum=2048, step=1, value=512,
                                                 label="Face Crop Resolution")
                max_crop_size = gr.Slider(minimum=0, maximum=2048, step=1, value=1024, label="Max Crop Size")
                face_denoising_strength = gr.Slider(minimum=0.00, maximum=1.00, step=0.01, value=0.5,
                                                    label="Face Denoising Strength")
                face_area_magnification = gr.Slider(minimum=1.00, maximum=10.00, step=0.01, value=1.5,
                                                    label="Face Area Magnification ")
        ...
        return [project_dir, generation_test, mask_mode, inpaint_area, use_depth, img2img_repeat_count, inc_seed,
                auto_tag_mode, add_tag_to_head, add_tag_replace_underscore, is_facecrop, face_detection_method,
                face_crop_resolution, max_crop_size, face_denoising_strength, face_area_magnification,
                enable_face_prompt, face_prompt, controlnet_weight, controlnet_weight_for_face,
                disable_facecrop_lpbk_last_time, use_preprocess_img]
    ...
    def detect_face(self, img, mask, face_detection_method, max_crop_size):
        ...
        for face in faces:
            ...
            face_coords.append(
                [x / img_array.shape[1], y / img_array.shape[0], w / img_array.shape[1], h / img_array.shape[0]])
    ...
    def get_mask(self):
        def create_mask(output, x_rate, y_rate, k_size):
            ...
            img = cv2.ellipse(img, ((256, 256), (int(512 * x_rate), int(512 * y_rate)), 0), (255, 255, 255),
                              thickness=-1)
    ...
    def face_crop_img2img(self, p, face_coords, face_denoising_strength, face_area_magnification, enable_face_prompt,
                          face_prompt, controlnet_input_img, controlnet_input_face_imgs, preprocess_img_exist):
        ...
        for face, coord, controlnet_input_face in zip(face_imgs, new_coords, controlnet_input_face_imgs):
            ...
            face_proc = self.process_images(face_p, controlnet_input_face, self.controlnet_weight_for_face,
                                            preprocess_img_exist)
    ...
    def interrogate_clip(self, imgs, masks):
        ...
        try:
            ...
            for img, mask, caption in zip(imgs, masks, caption_list):
                ...
                clip_image = shared.interrogator.clip_preprocess(image).unsqueeze(0).type(shared.interrogator.dtype).to(
                    devices.device_interrogate)

                res = ""

                with torch.no_grad(), devices.autocast():
                    image_features = shared.interrogator.clip_model.encode_image(clip_image).type(
                        shared.interrogator.dtype)
    ...
    def run(self, p, project_dir, generation_test, mask_mode, inpaint_area, use_depth, img2img_repeat_count, inc_seed,
            auto_tag_mode, add_tag_to_head, add_tag_replace_underscore, is_facecrop, face_detection_method,
            face_crop_resolution, max_crop_size, face_denoising_strength, face_area_magnification, enable_face_prompt,
            face_prompt, controlnet_weight, controlnet_weight_for_face, disable_facecrop_lpbk_last_time,
            use_preprocess_img):
        args = locals()
        ...
        with open(os.path.join(project_dir if is_invert_mask == False else inv_path,
                               time.strftime("%Y%m%d-%H%M%S_") + "prompts.txt"), "w") as f:
            f.write(json.dumps(prompts_dict, indent=4))

        ######################
        # img2img
        for img, mask, controlnet_input_img, face_coords, prompts in zip(imgs, masks, controlnet_input_imgs,
                                                                         face_coords_dict.values(),
                                                                         prompts_dict.values()):
            ...
            if mask_mode != "None" or use_depth:
                if use_depth:
                    depth_found, _p.image_mask = self.get_depth_map(mask_image, depth_path, img_basename,
                                                                    is_invert_mask)
            ...
            if face_coords:
                controlnet_input_face_imgs, _ = self.face_img_crop(controlnet_input_base_img, face_coords,
            ...
            while repeat_count > 0:
                ...
                if face_coords:
                    proc = self.face_crop_img2img(_p, face_coords, face_denoising_strength, face_area_magnification,
                                                  enable_face_prompt, face_prompt, controlnet_input_base_img,
                                                  controlnet_input_face_imgs, preprocess_img_exist)
                else:
                    proc = self.process_images(_p, controlnet_input_base_img, self.controlnet_weight,
                                               preprocess_img_exist)
                    print(proc.seed)
